...
**Goal:** Build a zero-dependency, cross-platform coding-assistant CLI (`oa`) powered by the published `openagentic_sdk` package (default provider: RIGHTCODE), with a multi-turn REPL that supports colors on PowerShell/Linux/macOS.

**Architecture:** Implement a new Python package `openagentic_cli/` in this repo (installed alongside `openagentic_sdk`). Use `argparse` for subcommands, a small config layer that maps flags/env → `OpenAgenticOptions`, and reuse `OpenAgentSDKClient` + `ConsoleRenderer` for streaming output. Add a tiny ANSI styling layer that auto-enables color (TTY) and enables VT processing on Windows when possible.

**Tech Stack:** Python 3.11+, stdlib only (`argparse`, `asyncio`, `json`, `pathlib`, `ctypes` on Windows), `openagentic_sdk` (imported as a normal installed package), `unittest`.

---

### Task 1: Add `openagentic_cli/` package scaffold

**Files:**
- Create: `openagentic_cli/__init__.py`
- Create: `openagentic_cli/__main__.py`
- Create: `openagentic_cli/args.py`
- Create: `openagentic_cli/config.py`
...
- `openagentic_cli/__init__.py` sets `__all__` minimally.
- `openagentic_cli/__main__.py` defines `main(argv: list[str] | None = None) -> int` and calls it under `if __name__ == "__main__":`.
- `openagentic_cli/args.py` defines `build_parser()` returning an `argparse.ArgumentParser` with subcommands: `chat`, `run`, `resume`, `logs`.
- `openagentic_cli/config.py` defines helpers to build `OpenAgenticOptions` (RIGHTCODE defaults).
...
**Files:**
- Create: `openagentic_cli/style.py`
...
### Task 3: Implement config layer (RIGHTCODE defaults; published `openagentic_sdk` import)
...
In `openagentic_cli/config.py` implement:
...
- `build_options(cwd, project_dir, permission_mode, allowed_tools, session_root, resume, interactive)` returning `OpenAgenticOptions` with:
...
**Files:**
- Create: `openagentic_cli/repl.py`
...
- `run_chat(options, *, color_config, debug: bool, stdin, stdout) -> int`:
  ...
  - Commands:
    ...
    - `/skills` prints local index via `openagentic_sdk.skills.index.index_skills(project_dir=...)` (no model call)
  ...
  - Uses `ConsoleRenderer` for assistant output, but prints status lines with ANSI styling from `openagentic_cli/style.py`.
- Ensure Ctrl+C interrupts the in-flight request (call `client.interrupt()` then continue loop).

In `openagentic_cli/__main__.py`:
...
Run:
- `RIGHTCODE_API_KEY=... python -m openagentic_cli chat`
Expected:
...
**Files:**
- Modify: `openagentic_cli/__main__.py`
- Create: `openagentic_cli/run_cmd.py`
...
- `async run_once(options, prompt, *, stream: bool, json_output: bool) -> int`:
  - For `stream=True`, use `openagentic_sdk.console.console_query(...)`
  - For `stream=False`, use `openagentic_sdk.run(...)` and print final text (or JSON).
...
**Files:**
- Create: `openagentic_cli/logs_cmd.py`
- Modify: `openagentic_cli/__main__.py`
...
In `openagentic_cli/logs_cmd.py`:
...
- In `pyproject.toml`:
  - Add `openagentic_cli*` to package discovery.
  - Add `[project.scripts] oa = "openagentic_cli.__main__:main"`.
